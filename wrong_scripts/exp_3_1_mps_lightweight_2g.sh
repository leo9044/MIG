...
RESULT_DIR="${RESULT_DIR:-/home/leo/MIG/experiments/fresh_20260830/exp_3_1_light_2g}"
IMAGE="${IMAGE:-nvcr.io/nvidia/pytorch:25.08-py3}"
RUN_SECONDS="${RUN_SECONDS:-20}"
mkdir -p "$RESULT_DIR"

echo "=== Fresh experiment 3-1 (Lightweight on 2g): MPS spatial sharing ==="

nvidia-smi -L | grep -q 'MIG 2g\.0gb' || {
  echo "MIG 2g instance missing; recreating it."
...
[[ -n "$uuid_2g" ]] || { echo "A 2g MIG UUID is required." >&2; exit 1; }
...
start_mps
run_four_workers_2g "mps_on"
stop_mps
run_four_workers_2g "mps_off"
