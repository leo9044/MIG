...
RESULT_DIR="${RESULT_DIR:-/home/leo/MIG/experiments/fresh_20260830/exp_3_1_nomig_mps}"
IMAGE="${IMAGE:-nvcr.io/nvidia/pytorch:25.08-py3}"
RUN_SECONDS="${RUN_SECONDS:-20}"
mkdir -p "$RESULT_DIR"

echo "=== Fresh experiment 3-1 (Non-MIG): MPS spatial sharing on bare-metal GPU ==="

sudo nvidia-smi mig -dci || true
sudo nvidia-smi mig -dgi || true
sudo nvidia-smi -mig 0 || true
sleep 5
...
run_four_workers_nomig() {
  local label="$1"
  echo "[mps] ${label} on bare-metal GPU"
...
start_mps
run_four_workers_nomig "mps_on_nomig"
stop_mps
run_four_workers_nomig "mps_off_nomig"
